...
import { normalizeSupabaseUrl } from '../security/urlNormalize.js';
...
/** Normalise a base URL: trim spaces and strip ALL trailing slashes, plus fix
 *  common paste mistakes (missing scheme, pasted /functions/v1 path, etc). */
function baseUrl(url: string): string {
  return normalizeSupabaseUrl(url).url;
}
...
export async function checkBackend(cfg: Pick<WizardConfig, 'supabaseUrl'>): Promise<CheckResult> {
  ...
  try {
    const res = await fetch(`${baseUrl(cfg.supabaseUrl)}/auth/v1/health`, { method: 'GET' });
...
export async function checkToken(
  ...
  try {
    const res = await fetch(`${baseUrl(cfg.supabaseUrl)}/functions/v1/agent-gateway`, {
      method: 'POST',
      headers: {
    ...
    if (res.status === 404) {
      return { ok: false, message: 'agent-gateway haipatikani kwenye System URL uliyoweka. Hakikisha System URL ni sahihi (https://PROJECT.supabase.co, bila herufi za ziada) na agent-gateway imedeploy.' };
    }
